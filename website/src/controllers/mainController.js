...
const mainC = {
  ...
  loginProcess: (req, res) => {
    ...
    if (req.body.soyEmpleado) {
      Employs.findByPk(user).then((employ) => {
        if (employ) {
          let okPassE = bcryptjs.compareSync(
          ...
          if (okPassE) {
            ...
            res.cookie("adminRegister", req.body.usernameLogin, {
              maxAge: 1000 * 60 * 10,
            });
            return res.redirect("/");
          } else {
      ...
      Clients.findByPk(user).then((cliente) => {
        if (cliente) {
          ...
          if (okPass) {
            ...
            res.cookie("clientRegister", req.body.usernameLogin, {
              maxAge: 1000 * 60 * 10,
            });
            return res.redirect("/");
          } else {
